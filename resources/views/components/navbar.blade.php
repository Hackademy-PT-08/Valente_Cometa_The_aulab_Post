    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarColor01">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0  ">
                ...
                @auth
                <li class="nav-item btn ">
                    <a class="nav-link" href="{{route('articles.create')}}"><i class="bi bi-pencil-fill"></i> {{__('ui.pubblica')}}</a>
                ...
                <li class="nav-item dropdown btn ">
                    ...
                    <ul class="dropdown-menu btn btn-dark">
                        <li><a class="dropdown-item" href="{{route('work.with.us')}}">{{__('ui.work')}}</a></li>
                        @auth
                        @if (Auth::user()->is_admin)
                        <li><a class="dropdown-item" href="{{route('admin.dashboard')}}">{{__('ui.personal')}}</a></li>
                        @endif
                        @if (Auth::user()->is_revisor)
                        <li><a class="dropdown-item" href="{{route('revisor.dashboard')}}">{{__('ui.area')}}</a></li>
                        @endif
                        @endauth
                        @if(Auth::user() && Auth::user()->is_writer)
                        <li><a class="dropdown-item" href="{{route('article.dashboard')}}">{{__('ui.area1')}}</a></li>
                        @endif
            ...
            <button type="button" class="btn btn-dark "><a class="nav-link"> {{__('ui.welcome')}}
